	...
	repeat(array_length(op.party))
	{
		if (go[i])
		{
			...
			if (stickOffset[i] >= 0)
			{
				...
				//print(stickX[i],stickX[i],i1[1]+19*i,,,,0.5,0.5,,c_red);
				if (stickOffset[i] == 0) { if (stickX[i] >= 42) { accuracy=42/stickX[i]; }else{ accuracy=stickX[i]/(42*2); } /*print(accuracy,stickX[i],i1[1]+19*i,,,,0.5,0.5,,c_red);*/ }
			}
		}
		++i;
	}
...
//draw_ext(ss1,0,0,0,0.5,0.5,,,0.5+dsin(op.time*7)*0.5);