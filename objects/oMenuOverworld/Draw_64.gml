if (time > 0)
{
	...
	if (page == "equip1" or page == "equipped" or page == "equipped" or page == "weaarm")
	{
		...
		if (op.equipped[op.overworldMenuSaveCursor[2]][0] != -1)
		{
			if (op.equipped[op.overworldMenuSaveCursor[2]][0]._imageIndex == -1)
				draw_ext(sSymbols2,op.party[op.overworldMenuSaveCursor[2]]._numberExistence,171.5,60);
			
			if (op.equipped[op.overworldMenuSaveCursor[2]][0]._imageIndex != -1)
				draw_ext(sSymbols3,op.equipped[op.overworldMenuSaveCursor[2]][0]._imageIndex,171.5,60);
		}
		if (op.equipped[op.overworldMenuSaveCursor[2]][1] != -1) { draw_ext(sSymbols3,clamp(op.equipped[op.overworldMenuSaveCursor[2]][1]._imageIndex,0,999),171.5,75); }
		if (op.equipped[op.overworldMenuSaveCursor[2]][2] != -1) { draw_ext(sSymbols3,clamp(op.equipped[op.overworldMenuSaveCursor[2]][2]._imageIndex,0,999),171.5,90); }
